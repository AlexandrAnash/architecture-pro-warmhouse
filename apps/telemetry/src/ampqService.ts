...
const NAME_QUEUE = 'telemetry-queue';
...
(async () => {
    ...
    const q = await ch1.assertQueue(NAME_QUEUE);
    ch1.bindQueue(q.queue, 'smarthome', 'telemetry.reading')
    
    ch1.consume(q.queue, (msg) => {
        if (msg !== null) {
            console.log('Received:', msg.content.toString());
            const device = JSON.parse(msg.content.toString());
            registryTelemetry.set(device.id, device);
            ch1.ack(msg);
        } else {
